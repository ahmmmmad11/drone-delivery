...
describe('Test droneService', function() {
    ...
    describe('Full workflow integration tests', () => {
        it('should list drones with pagination', async () => {
            ...
            const result = await droneService.getDrones({}, 1, 3);
        ...
        it('should handle drone lifecycle: create -> set broken -> set fixed', async () => {
            ...
            // Set to broken
            drone = await droneService.setBroken(drone.id);
            expect(drone.status).to.equal(droneStatus.BROKEN);

            // Set to fixed (active)
            drone = await droneService.setFixed(drone.id);
            expect(drone.status).to.equal(droneStatus.ACTIVE);
        });
        ...
        it('should handle order workflow with delivery', async () => {
            ...
            // Get current order
            const currentOrder = await droneService.getCurrentDroneOrder(drone.id);
        ...
        it('should update order location during delivery', async () => {
            ...
            await droneService.updateCurrentOrderLocation(drone.id, newLocation);
        ...
        it('should drop current order when drone breaks', async () => {
            ...
            await droneService.setBroken(drone.id);
        ...
        it('should filter drones by status', async () => {
            ...
            const activeDrones = await droneService.getDrones(
                { status: droneStatus.ACTIVE },
                1,
                10
            );
    ...
    describe('Edge cases and error handling', () => {
        it('should handle updating location for non-existent order', async () => {
            ...
            const result = await droneService.updateCurrentOrderLocation(drone.id, location);
        ...
        it('should handle empty database queries', async () => {
            const result = await droneService.getDrones({}, 1, 10);
